header Kernel
  ...
  enum FILE, TERMINAL, PIPE_R, PIPE_W                     -- Kinds of OpenFile
  ...
  functions
    ...
    Handle_Sys_Yield ()
    Handle_Sys_Exec (filename: ptr to array of char, comArgs: ptr to array of ptr to array of char) returns int
    ...
    Handle_Sys_Pipe (fdArray: ptr to array [2] of int) returns int
    ...
    ProcessFinish (exitStatus: int)
	  InitFirstProcess()						                    --Dane added
	  StartUserProcess(arg: int)						            --Dane added
    ResumeChildAfterFork(initPC: int)                	--Dane added
    ...
    external BecomeUserThread (initStack, initPC, initSystemStack, comArgs: int)
  ...
  class FrameManager
    ...
    methods
      ...
      --PutAFrame (frameAddr: int)                    -- Implement for pipes
  ...
  class OpenFile
    superclass Listable
    fields
      kind: int                      -- FILE, TERMINAL, PIPE_R or PIPE_W
      ...
      pipeptr: ptr to Pipe           -- pointer to Pipe that manages it
    methods
      Print ()
...
----------------------------- Pipe -------------------------
  ...
  class Pipe
    superclass Listable
    fields
      numReferences: int --Number of references to pipe object 0, 1 or 2 (close if 0)
